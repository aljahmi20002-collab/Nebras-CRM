#!/usr/bin/env python3
"""Copy an existing NebrasCRM SQLite database into MySQL or MariaDB.
...
The target MySQL-compatible schema is initialized automatically. Global geography
is not copied from SQLite: it is rebuilt from bundled GeoNames data.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Migrate NebrasCRM SQLite data to MariaDB")
    parser.add_argument("--source", default="crm.db", help="path to source SQLite database")
    parser.add_argument(
        "--replace", action="store_true",
        help="delete existing importable MariaDB data before importing (required for a clean migration)",
    )
...
def main():
    ...
    if os.environ.get("CRM_DB_ENGINE", "").lower() not in {"mariadb", "mysql"}:
        raise SystemExit("Set CRM_DB_ENGINE=mariadb or mysql and CRM_DB_HOST/PORT/NAME/USER/PASSWORD first.")
    if not args.replace:
        raise SystemExit("Pass --replace after backing up MariaDB to avoid mixing existing data.")

    # Import after configuration validation so main.py creates the MariaDB schema.
    import db as D
    import main as APP
    ...
    target.commit()
    # No-op for MariaDB; kept alongside the PostgreSQL migration path so the
    # shared import flow remains explicit about identity synchronization.
    D.sync_identity(target, tables)
    target.commit()
    source.close()
    print(f"Migration complete: {total_rows} rows copied into MySQL/MariaDB.")
