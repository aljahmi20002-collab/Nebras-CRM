...
## قاعدة اليمن الجغرافية — `geo.py`
تسلسل إداري رسمي كامل مستورد من مصادر مفتوحة موثقة (Open Admin Data + Yemen-info):

| المستوى | العدد | المصدر |
|---|---|---|
| **محافظة** | 22 | رسمي بالعربي والإنجليزي مع الإحداثيات ومفتاح الهاتف |
| **مديرية** | 335 | رسمي |
| **عزلة** | 2,234 | رسمي |
| **قرية** | 41,494 | رسمي |
| **حارة** | يُدار بالنظام | لا يغطيه أي مصدر مفتوح |
| **شارع** | يُدار بالنظام | لا يغطيه أي مصدر مفتوح |

**إجمالي 44,085 موقعاً حقيقياً.** يشمل: متصفح هرمي بأربعة أعمدة متتابعة، بحث فوري عبر كل المستويات، ربط كل عميل بموقعه الدقيق، وتقرير أداء ومبيعات لكل محافظة.
...
## ملاحظات إنتاجية
قبل النشر الحقيقي: غيّر `SECRET` في `main.py` و`PSECRET` في `portal.py` و`WEBHOOK_SECRET` في `payments.py` و`ASECRET` في `agentportal.py`، واستبدل المزوّد الوهمي بمزوّد دفع حقيقي (لا تستقبل بيانات البطاقة على خادمك — استخدم صفحة المزوّد المستضافة لتفادي نطاق PCI)، بدّل تجزئة كلمة المرور إلى bcrypt، انقل إلى PostgreSQL، وأضف HTTPS + تحديد معدل الطلبات.
